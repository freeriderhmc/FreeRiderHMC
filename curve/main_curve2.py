...
for files in file_list:
    ...
    if frame_num == 0:
        ...
        l_out, l_in = -150, -10-int(steering_angle)
        r_out, r_in = 150-int(steering_angle)*5, 10
    else:
        l_out, l_in = -(150+int(steering_angle)*5), 10
        r_out, r_in = 150, 10-int(steering_angle)*5
    
    print(l_out , l_in,  r_in,r_out)
    ...
    # pc_leftlane = pc_lane[pc_lane[:][:,1]>1]
    # pc_leftlane = pc_leftlane[pc_leftlane[:][:,1]<4]
    # # pc_leftlane = left_lane[left_lane[:][:,0]<40]
    # pc_rightlane = pc_lane[pc_lane[:][:,1]<-1]
    # pc_rightlane = pc_rightlane[pc_rightlane[:][:,1]>-4]
    ...
    elif frame_num == 0: pre_steering_angle = steering_angle
