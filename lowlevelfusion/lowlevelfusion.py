...
path = "/media/jinyoung/Samsung_T5/"
path_lidar = path + "0st-test_lidar/"
path_csv = path + "csvdata/"
path_image = path + "0st-test_img/"
path_semanticMap = path + "0st-test_bin/"
...
for files in file_list:
    ...
    pc_front = data_front[(data_front[:,2] >= -1.3)]
    pc_back = data_back[(data_back[:,2] >= -1.3)]
    ...
    labels = np.asanyarray(cloud.cluster_dbscan(0.7,3))
    ...
    for i in range(np.max(labels)+1):
        ...
        # Car : 55, bus : 54, Lane : 24, Human : 19, Bicycle : 52, forest : 30, else : 0 and else, 
        if classification == 55 or classification == 54:
