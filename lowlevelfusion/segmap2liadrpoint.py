...
semanticMap = np.fromfile(path_semanticMap + map_list[0], dtype = np.int64)
semanticMap = semanticMap.reshape(376, 1241)

# files = file_list[0]
# semanticMap = map_list[0]
#print(files)
pointcloud = np.fromfile(path_lidar+file_list[0],dtype = np.float32)
...
# print(Point_y)
start = time.time()
label_front = np.array(list(map(lambda x : semanticMap[x[1], x[0]] if 0 <= x[0] < 1241 and 0 <= x[1] < 376 else 0, Y)))
