...
# path = '/media/yimju/외장SSD/train_dataset_kitti/interchange/'
# path = '/media/yimju/Samsung_T5/kitti_for_train/hope/'
...
###############################################################################################################
    ...
    l_out, r_out, l_in, r_in = -600, 600, -10, 10
    # if frame_num == 0:
    #     l_out, r_out, l_in, r_in = -500, 500, -10, 10
    
    # #right
    # elif steering_angle<0:
    #     l_out, l_in = -100, -10-int(steering_angle)
    #     r_out, r_in = 50-int(steering_angle)*10, 10-int(steering_angle*2)
    # #left
    # else:
    #     l_out, l_in = -(100+int(steering_angle)*5), -10-int(steering_angle*2)
    #     r_out, r_in = 50, 10-int(steering_angle)*5
    # print(l_out, l_in, r_out, r_in)

    for height in range(h):
        for width in range(w):
            if  middle + l_out <width< middle + l_in and height >150:
                if semanticMap[height,width-1] == 24 and semanticMap[height,width] == 24 and semanticMap[height,width+1] != 24 and flag_leftget == False: 
                    ...
                    left_semanticMap[height,width-1] = 24
                    flag_leftget = True
                else: left_semanticMap[height,width] = 0
            else: left_semanticMap[height,width] = 0

            if middle + r_in < width < middle + r_out and height >150:
            ...
            # if  middle + l_in < width < middle + r_in and 250<height<h:
    ...
    # pc_leftlane = pc_lane[pc_lane[:][:,1]>l_in]
    # pc_leftlane = pc_leftlane[pc_leftlane[:][:,1]<l_out]
    # # pc_leftlane = left_lane[left_lane[:][:,0]<40]
    # pc_rightlane = pc_lane[pc_lane[:][:,1]<r_in]
    # pc_rightlane = pc_rightlane[pc_rightlane[:][:,1]>r_out]
    # # right_lane = right_lane[right_lane[:][:,0]<40]

    # stop_lane = pc_lane[pc_lane[:][:,1]<l_in]
    # stop_lane = stop_lane[stop_lane[:][:,1]>r_in]
    # stop_lane = stop_lane[stop_lane[:][:,0]<10]
    # if len(stop_lane)>20: stop = True
    # else: stop = False

    stop_lane = pc_lane[pc_lane[:][:,1]<l_in-0.5]
    stop_lane = stop_lane[stop_lane[:][:,1]>r_in+0.5]
    stop_lane = stop_lane[stop_lane[:][:,0]<10]
    ...
    if frame_num == 0: l_out, r_out, l_in, r_in = 10, -10, -2, 2
    elif steering_angle<0:
        l_out, l_in = d_out+int(steering_angle/20), d_in+int(steering_angle/10)
        r_out, r_in = -d_out+int(steering_angle/10), -d_in+int(steering_angle/10)
    else:
        l_out, l_in = (d_out+int(steering_angle/10)), d_in-int(steering_angle/10)
        r_out, r_in = -d_out-int(steering_angle/20), -d_in+int(steering_angle/10)
    print(l_out,l_in,r_out,r_in)
    
    pc_leftlane = pc_leftlane[pc_leftlane[:][:,1]>l_in]
    pc_leftlane = pc_leftlane[pc_leftlane[:][:,1]<l_out]
    # pc_leftlane = left_lane[left_lane[:][:,0]<40]
    pc_rightlane = pc_rightlane[pc_rightlane[:][:,1]<r_in]
    pc_rightlane = pc_rightlane[pc_rightlane[:][:,1]>r_out]

    if pre_steering_angle>5: x_limit = 15
    else: x_limit = 20

    pc_leftlane = pc_leftlane[pc_leftlane[:][:,0]<x_limit]
    pc_rightlane = pc_rightlane[pc_rightlane[:][:,0]<x_limit]

    # plt.plot(pc_leftlane[:,0], pc_leftlane[:,1], 'bo', markersize = 2)
    # plt.plot(pc_rightlane[:,0], pc_rightlane[:,1], 'ro', markersize = 2)
    left_lane, right_lane, left_fit ,right_fit, steering_angle = curve(pc_leftlane,pc_rightlane,stop)

    if frame_num != 0 and ((abs(pre_steering_angle)<10 and abs(pre_steering_angle - steering_angle)>20) or(abs(pre_steering_angle)>=10 and abs(pre_steering_angle - steering_angle)>20)):
        left_lane = pre_left_lane
        right_lane = pre_right_lane
        left_fit = pre_left_fit
        right_fit = pre_right_fit
        steering_angle = pre_steering_angle
    else: pre_steering_angle = steering_angle
